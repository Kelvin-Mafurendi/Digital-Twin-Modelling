...
{
  EALLOW;
  AdcdRegs.ADCSOC0CTL.bit.CHSEL = 14U; /* Set SOC0 channel select to ADCIN14*/
  AdcdRegs.ADCSOC0CTL.bit.TRIGSEL = 0U;
  AdcdRegs.ADCSOC0CTL.bit.ACQPS = (uint16_T)14.0;
                               /* Set SOC0 S/H Window to 15.0 ADC Clock Cycles*/
  AdcdRegs.ADCINTSOCSEL1.bit.SOC0 = 0U;
                                   /* SOCx No ADCINT Interrupt Trigger Select.*/
  AdcdRegs.ADCOFFTRIM.bit.OFFTRIM = (uint16_T)AdcdRegs.ADCOFFTRIM.bit.OFFTRIM;/* Set Offset Error Correction Value*/
  AdcdRegs.ADCCTL1.bit.INTPULSEPOS = 1U;
             /* Interrupt pulse generation occurs at the end of the conversion*/
  AdcdRegs.ADCSOCPRICTL.bit.SOCPRIORITY = 0U;/* All in round robin mode SOC Priority*/
  EDIS;
}
...
void InitAdcD(void)
{
  ...
  AdcdRegs.ADCCTL2.bit.PRESCALE = 8U;
  AdcSetMode((uint16_T)ADC_ADCD, (uint16_T)ADC_RESOLUTION_12BIT,(uint16_T)
             ADC_SIGNALMODE_SINGLE);
